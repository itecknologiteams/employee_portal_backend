...
export async function getApprovedByIt(req, res) {
  try {
    const employeeId = await getEmployeeIdByCode(req.params.employeeCode)
    if (!employeeId) return res.status(404).json({ error: 'Employee not found' })
    const result = await requisitionService.getApprovedByIt(employeeId)
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(Array.isArray(result) ? result : [])
  } catch (error) {
    console.error('Approved by IT error:', error)
    res.status(500).json({ error: 'Failed to fetch approved requisitions' })
  }
}
...
// Example: Approved by HR Controller
export async function getApprovedByHR(req, res) {
  try {
    ...
    // --- NEW SECURITY CHECK ---
    // Pehle check karo kya ye banda HR hai?
    const isHr = await reqRepo.isHrMember(employeeId); 
    if (!isHr) {
      return res.json([]); // Agar HR nahi hai, toh chup-chap khali list bhej do
    }
    // ---------------------------
...
export async function getPendingIt(req, res) {
  try {
    const employeeId = await getEmployeeIdByCode(req.params.employeeCode)
    if (!employeeId) return res.status(404).json({ error: 'Employee not found' })
    const result = await requisitionService.getPendingIt(employeeId)
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(Array.isArray(result) ? result : [])
  } catch (error) {
    console.error('Pending IT error:', error)
    res.status(500).json({ error: 'Failed to fetch pending requisitions' })
  }
}

export async function approveIt(req, res) {
  try {
    const result = await requisitionService.approveIt(req.body)
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json({ message: result.message, status: result.status })
  } catch (error) {
    console.error('IT approve error:', error)
    res.status(500).json({ error: 'Failed to update approval' })
  }
}
...
export async function flagItemUnavailable(req, res) {
  try {
    const result = await requisitionService.flagItemUnavailable({ ...req.body, itemId: req.params.itemId })
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(result)
  } catch (error) {
    console.error('Flag item unavailable error:', error)
    res.status(500).json({ error: 'Failed to flag item' })
  }
}

export async function restoreFlaggedItem(req, res) {
  try {
    const result = await requisitionService.restoreFlaggedItem({ ...req.body, itemId: req.params.itemId })
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(result)
  } catch (error) {
...
export async function getItemReviewQueue(req, res) {
  try {
    const employeeId = await getEmployeeIdByCode(req.params.employeeCode)
    if (!employeeId) return res.status(404).json({ error: 'Employee not found' })
    const result = await requisitionService.getItemReviewQueue(employeeId)
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(Array.isArray(result) ? result : [])
  } catch (error) {
    console.error('Item review queue error:', error)
    res.status(500).json({ error: 'Failed to fetch item review queue' })
  }
}

export async function reviewFlaggedItem(req, res) {
  try {
    const result = await requisitionService.reviewFlaggedItem({ ...req.body, itemId: req.params.itemId })
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(result)
  } catch (error) {
    console.error('Review flagged item error:', error)
    res.status(500).json({ error: 'Failed to review item' })
  }
}

export async function reviseRequisition(req, res) {
  try {
    const result = await requisitionService.reviseRequisition({ ...req.body, reqId: req.params.reqId })
    if (result.error) return res.status(result.status).json({ error: result.error })
    res.json(result)
  } catch (error) {
    console.error('Revise requisition error:', error)
    res.status(500).json({ error: 'Failed to revise requisition' })
  }
}

export async function getRequisitionItemEvents(req, res) {
  try {
    const result = await requisitionService.getRequisitionItemEvents(req.params.reqId)
    res.json(Array.isArray(result) ? result : [])
  } catch (error) {
    console.error('Item events error:', error)
    res.status(500).json({ error: 'Failed to fetch item events' })
  }
}
